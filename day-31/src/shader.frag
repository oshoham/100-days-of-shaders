...
float scene_sdf(vec3 sample_point) {
  vec3 box_point = (rotate_x(-u_time) * rotate_y(-u_time * 0.5) * vec4(sample_point, 1.0)).xyz;
  return box_sdf_fast(box_point, vec3(0.0), vec3(
    map(sin(u_time * 2.0), -1.0, 1.0, 1.0, 2.0),
    map(sin(u_time * 2.0 + PI), -1.0, 1.0, 1.0, 2.0),
    2.0
  ));
}
