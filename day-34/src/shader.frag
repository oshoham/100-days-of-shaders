...
vec3 twist(vec3 point, float scale) {
...
float scene_sdf(vec3 sample_point) {
  // sample_point = (rotate_y(u_time) * vec4(sample_point, 1.0)).xyz;
  return 0.25 * box_sdf(twist(sample_point, map(sin(3.0 / 2.0 * PI + u_time), -1.0, 1.0, 0.0, 5.0)), vec3(0.0), vec3(2.0));
}
